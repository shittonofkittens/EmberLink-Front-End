...
export default function ChatBox({ initialRoom = "forge" }) {
  ...
  useEffect(() => {
    ...
    setCurrentRoom(activeRoom); // Not strictly needed, but okay to keep
  ...
  for (const soulId of getStaggeredOrder(soulTargets, roomMode)) {
    ...
    try {
      ...
      if ((isVoiceMode || showSoulCall) && voiceUrl) {
        console.log("🎧 Attempting to play voice URL:", voiceUrl);
        ...
        if (isLikelyBase64) {
          console.log("🎙️ Voice URL is base64 — creating blob");
          const audioBlob = base64ToBlob(voiceUrl, "audio/mpeg");
          const blobUrl = URL.createObjectURL(audioBlob);
          audio = new Audio(blobUrl);
        } else if (voiceUrl && voiceUrl.startsWith("blob:")) {
          console.warn("❌ Voice URL is a fake blob — skipping playback.");
          return;
        } else if (voiceUrl && voiceUrl.startsWith("http")) {
          console.log("🌐 Voice URL is HTTP — using directly");
          audio = new Audio(voiceUrl);
        } else {
          console.warn("⚠️ Invalid or missing voice URL:", voiceUrl);
          return;
        }
        ...
        console.warn("⚠️ Skipping voice playback — invalid or missing voice URL:", voiceUrl);
      }
